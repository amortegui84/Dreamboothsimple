...
class SetupTraining:
    ...
    def submit_form_click(self, b):
        with self.output:
            ...
            config.saturate(
                ...
                max_training_steps=self.max_training_steps_input.value,
                learning_rate=self.learning_rate_select.value,
                class_word=self.class_word_input.value,
                flip_images=self.flip_slider.value,
                token=self.token_input.value,
                save_every_x_steps=self.save_every_x_steps_input.value,
                ...
                model_type=self.selected_model.model_type,
                model_path=self.selected_model.path,
                save_path=os.path.join(self.config_save_path, self.project_name_input.value),
            )
    ...
    def handle_training_images(self, uploaded_training_images):
        # create the training_images folder if it doesn't exist
        if not os.path.exists(self.training_images_save_path):
            os.makedirs(self.training_images_save_path)

        for name, file in uploaded_training_images.items():
            with open(os.path.join(self.training_images_save_path, name), "wb") as fp:
                fp.write(file["content"])

    def download_regularization_images(self, dataset_name):
        # create the regularization_images folder if it doesn't exist
        regularization_images_path = f"./regularization_images/{dataset_name}"
        if not os.path.exists(regularization_images_path):
            os.makedirs(regularization_images_path)

            # Download the regularization images zip file
            regularization_images_zip_url = 'https://drive.google.com/uc?id=1eHQ0HsAedj_9l9bI3g8QjKQ5GoRxhSze'
            regularization_images_zip_path = f"./regularization_images/{dataset_name}.zip"
            gdown.download(regularization_images_zip_url, regularization_images_zip_path, quiet=False)

            # Extract the regularization images zip file
            with zipfile.ZipFile(regularization_images_zip_path, 'r') as zip_ref:
                zip_ref.extractall(regularization_images_path)

            # Remove the regularization images zip file
            os.remove(regularization_images_zip_path)

        return regularization_images_path
